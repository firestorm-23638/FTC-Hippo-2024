...
public class Drivetrain extends SubsystemBase {
    ...
    public void periodic() {
        ...
        telemetry.addData("Robot X", this.mecanumDrive.localizer.getPose().position.x);
        telemetry.addData("Robot Y", this.mecanumDrive.localizer.getPose().position.y);
        telemetry.addData("Robot Heading", Math.toDegrees(this.mecanumDrive.localizer.getPose().heading.log()));
    }
    ...
    public void setRawPower(double forwardSpeed, double strafeSpeed, double turnSpeed) {
        this.fieldCentricDrive(forwardSpeed, strafeSpeed, turnSpeed, 0.0);
    }

    public void setCurrentPose(Pose2d newPose) {
        this.mecanumDrive.localizer.setPose(newPose);
    }

    public Pose2d getCurrentPose() {
        return this.mecanumDrive.localizer.getPose();
    }

    public void driveFieldCentric(double forwardSpeed, double strafeSpeed, double turnSpeed, boolean isFieldCentric) {
        if (isFieldCentric) {
            this.fieldCentricDrive(forwardSpeed, strafeSpeed, turnSpeed,Math.toDegrees(mecanumDrive.localizer.getPose().heading.log()) + 90);
        }
        else {
