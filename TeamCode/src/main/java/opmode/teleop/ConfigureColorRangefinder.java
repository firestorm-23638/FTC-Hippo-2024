...
public class ConfigureColorRangefinder extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ColorRangefinder crf =
                new ColorRangefinder(hardwareMap.get(RevColorSensorV3.class, "colorSensorConfig"));
        ...
        // for tuning hsv value
//        crf.setPin0Analog(ColorRangefinder.AnalogMode.HSV);
//        crf.setPin1Digital(ColorRangefinder.DigitalMode.HSV, 0, 20); // placeholder

        // YELLOW HSV: 86, 54.5, 6
        // RED HSV: 53, 30.8, 3.3
        // BLUE HSV: 202, 57.3, 3.8

        crf.setPin0Digital(ColorRangefinder.DigitalMode.HSV, 120 / 360.0 * 255, 250 / 360.0 * 255); // blue
        crf.setPin0Digital(ColorRangefinder.DigitalMode.HSV, 80 / 360.0 * 255, 100 / 360.0 * 255); // yellow
        crf.setPin0DigitalMaxDistance(ColorRangefinder.DigitalMode.HSV, 20); // 20mm or closer requirement
//
        crf.setPin1Digital(ColorRangefinder.DigitalMode.HSV, 0 / 360.0 * 255, 70 / 360.0 * 255); // red
        crf.setPin1Digital(ColorRangefinder.DigitalMode.HSV, 80 / 360.0 * 255, 100 / 360.0 * 255); // yellow
        crf.setPin1DigitalMaxDistance(ColorRangefinder.DigitalMode.HSV, 30); // 20mm or closer requirement

        waitForStart();

        while (opModeIsActive()) {
            telemetry.addData("RED", crf.emulator.getNormalizedColors().red);
            telemetry.addData("GREEN", crf.emulator.getNormalizedColors().green);
            telemetry.addData("BLUE", crf.emulator.getNormalizedColors().blue);

            telemetry.addData("Current Distance", crf.readDistance());
